    ...
    : ControllerBase
{
    ...
    public async Task<IResult> DiscordAuthorize()
    {
        ...
        try
        {
            member = await discordService.GetMemberById(ulong.Parse(userId));
        }
        catch (System.Exception e)
        {
            // Member情報の取得に失敗しても認証自体は継続する
            _logger.LogError(e, "Failed to fetch Discord member info. UserId: {UserId}", userId);
        }
        var discordDisplayName = !string.IsNullOrEmpty(member?.Nickname) ? member.Nickname : member?.Username;
        _logger.LogInformation(
            "User logged in. RemoteIpAddress: {RemoteIpAddress}, DiscordMemberName: {DiscordMemberName}, UserId: {UserId}",
            remoteIpAddress, discordDisplayName, userId);
