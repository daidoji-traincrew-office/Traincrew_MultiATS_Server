...
// データ配信はRPCではなくBroadcastScheduler(下記3.5)によるプッシュ(ReceiveData/ReceiveSignalData)で行う。
// Hubメソッドはクライアント→サーバー方向の操作(てこ操作等)のみを持つ。
[Authorize(
...
// 具体的なScheduler実装 (Scheduler/BroadcastScheduler.cs)
// 連動盤/TID/CTCP/司令卓/列車の5ハブへの配信を1つに統合したScheduler。
// 1ティックにつき IBroadcastSnapshotService.BuildAsync() で1回だけスナップショットを取得し、
// 各ハブへ ReceiveData / ReceiveSignalData をプッシュする(RPCではない)。
...
**主要なScheduler:**
- `BroadcastScheduler`: 連動/TID/CTCP/司令卓/列車データの定期配信（250ms間隔、`ReceiveData`によるプッシュ配信）
