...
public class Oud2ToTtcConverter
{
    ...
    private TTC_Train? ConvertRessya(Dictionary<string, object> ressya, bool isKudari, int operationNumber)
    {
        ...
        try
        {
            (originName, originId, originEkiIdx, _) = GetOperationStation(ressya, 'B', isKudari, trainNumber);
        }
        catch (OperationStationKeyNotFoundException ex)
        {
            Console.Error.WriteLine($"[WARN] {ex.Message} → この列車をスキップします");
            return null;
        }
        ...
        Dictionary<string, object> ressya, char suffix, bool isKudari, string trainNumber)
    {
        ...
        throw new OperationStationKeyNotFoundException(
...
/// <summary>
/// Operation*A/B キーが列車データ内に存在しない場合の例外
/// </summary>
public sealed class OperationStationKeyNotFoundException(string message) : InvalidOperationException(message);
