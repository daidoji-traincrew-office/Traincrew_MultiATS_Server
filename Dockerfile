...
RUN cd ./Traincrew_MultiATS_Server && dotnet restore -a $TARGETARCH 
...
RUN cd ./Traincrew_MultiATS_Server && dotnet publish -a $TARGETARCH --no-restore -o /app
...
WORKDIR /opt
ENTRYPOINT ["/app/Traincrew_MultiATS_Server"]