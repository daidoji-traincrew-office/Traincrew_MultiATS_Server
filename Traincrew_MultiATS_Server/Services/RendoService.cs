    ...
    ILogger<RendoService> logger) : IRendoService
{
    ...
    public async Task LeverToRouteState()
    {
        ...
        // RouteLeverDestinationButtonを取得
        var routeLeverDestinationButtonList = await routeLeverDestinationRepository.GetByRouteIds(routeIds);
        // てこを取得
        var leverIds = routeLeverDestinationButtonList
        ...
        var leverByRouteId = routeLeverDestinationButtonList
            .ToDictionary(
                rdb => rdb.RouteId,
                rdb => leverById[rdb.LeverId]
            );
        var routesByLeverId = routeLeverDestinationButtonList
            .GroupBy(rdb => rdb.LeverId)
            .ToDictionary(
                g => g.Key,
                g => g.Select(rdb => routeById[rdb.RouteId]).ToList());
        // 着点ボタンを取得
        var destinationButtonNames = routeLeverDestinationButtonList
            .Select(rdb => rdb.DestinationButtonName)
        ...
        var interlockingObjectById = conditionObjects
            .Union(routeById.Values)
        ...
        foreach (var routeLeverDestinationButton in routeLeverDestinationButtonList)
        {
            ...
            var targetThrowOutControl = targetThrowOutControls.GetValueOrDefault(route.Id, []);
            ...
            // リレー状態を計算
            var relayState = CalculateLeverRelayState(
                route,
                routeLeverDestinationButton,
                lever,
                button,
                routeCentralControlLever,
                sourceThrowOutControl,
                targetThrowOutControl,
                lockCondition,
                signalControlCondition,
            ...
            // それぞれ現在と異なる場合、更新
            if (routeState.IsLeverRelayRaised == relayState.IsLeverRelayRaised
                && routeState.IsThrowOutXRelayRaised == relayState.IsXRelayRaised
                && routeState.IsThrowOutXRRelayRaised == relayState.IsThrowOutXRRelayRaised
                && routeState.IsThrowOutYSRelayRaised == relayState.IsThrowOutYSRelayRaised)
            {
                continue;
            }

            routeState.IsLeverRelayRaised = relayState.IsLeverRelayRaised;
            routeState.IsThrowOutXRelayRaised = relayState.IsXRelayRaised;
            routeState.IsThrowOutXRRelayRaised = relayState.IsThrowOutXRRelayRaised;
            routeState.IsThrowOutYSRelayRaised = relayState.IsThrowOutYSRelayRaised;
    ...
    /// <summary>
    /// 特定の進路に対するてこ反応リレーと総括リレーの状態を計算する
    /// </summary>
    /// <param name="route">対象進路</param>
    /// <param name="routeLeverDestinationButton">進路とてことボタンの関連情報</param>
    /// <param name="lever">対象てこ(てこナシの場合はnull)</param>
    /// <param name="button">対象ボタン(nullの可能性あり)</param>
    /// <param name="routeCentralControlLever">駅扱いてこ(nullの可能性あり)</param>
    /// <param name="sourceThrowOutControls">この進路に対する総括制御のリスト</param>
    /// <param name="targetThrowOutControls">この進路が総括制御する側の情報</param>
    /// <param name="lockCondition">鎖錠条件</param>
    /// <param name="signalControlConditions">信号制御条件</param>
    /// <param name="interlockingObjectById">全連動オブジェクトの辞書</param>
    /// <param name="routeById">進路ID→進路の辞書</param>
    /// <param name="leverByRouteId">進路ID→てこの辞書</param>
        ...
        Dictionary<ulong, List<ThrowOutControl>> allSourceThrowOutControls)
    {
        ...
        // この進路に対して「てこナシ」な総括制御
        var sourceThrowOutControlWithoutLever = sourceThrowOutControls
        ...
        else
        {
            ...
            // この進路に対して「てこあり」な総括制御
            var sourceThrowOutControlWithLever = sourceThrowOutControls
            ...
            if (sourceThrowOutControlWithLever.Count > 0)
            {
                ...
                var lockRouteStates = lockCondition
                    ...
                    .Select(l => interlockingObjectById[l.ObjectId])
                    .OfType<Route>()
                ...
                var signalControlLockTrackCircuitState = signalControlConditions
                    .OfType<LockConditionObject>()
                    .Select(l => interlockingObjectById[l.ObjectId])
                    .OfType<TrackCircuit>()
                    .Select(tc => tc.TrackCircuitState)
                    .ToList();
                isThrowOutYSRelayRaised =
            ...
            else
            {
                ...
                // この進路に対して総括制御「される」進路
                var targetThrowOutRoutes = targetThrowOutControls
                    .SelectMany(toc =>
                    {
                        // 進路IDから進路を取得
                ...
                var targetSourceThrowOutRoutes = targetThrowOutRoutes
                    .ToDictionary(
                        ...
                        targetRoute => allSourceThrowOutControls
                            .GetValueOrDefault(targetRoute.Id, [])
                            .Select(toc => routeById[toc.SourceId])
                            .Distinct()
                            .Where(r => r.Id != route.Id)
                            .ToList()
        ...
        if (IsLocked(lockCondition, interlockingObjectById))
        {
        ...
        return new()
        {
            IsLeverRelayRaised = isLeverRelayRaised,
            IsXRelayRaised = isXRelayRaised,
            IsThrowOutXRRelayRaised = isThrowOutXRRelayRaised,
            IsThrowOutYSRelayRaised = isThrowOutYSRelayRaised
        };
    }
    ...
    public async Task RouteRelayWithoutSwitchingMachine()
    {
        ...
        foreach (var routeId in routeIds)
        {
            ...
            var result = ProcessRouteRelayWithoutSwitchingMachine(route, directLockCondition, routeLockCondition,
                interlockingObjects);
    ...
    public async Task DirectionRelay()
    {
        ...
        foreach (var directionRouteId in directionRouteIds)
        {
            ...
            if (directionRoute.LSingleLockedLeverId.HasValue)
            {
                ...
                {
                    LSingleLockedLeverIdState =
                        (interlockingObjects[directionRoute.LSingleLockedLeverId.Value] as DirectionRoute)
                        .DirectionRouteState.IsLRelayRaised;
                }
            ...
            if (directionRoute.RSingleLockedLeverId.HasValue)
            {
                ...
                {
                    RSingleLockedLeverIdState =
                        (interlockingObjects[directionRoute.RSingleLockedLeverId.Value] as DirectionRoute)
                        .DirectionRouteState.IsLRelayRaised;
                }
    ...
    public async Task SignalControl()
    {
        ...
        foreach (var routeId in routeIds)
        {
            ...
            // この進路を総括制御「する」進路
            var sourceThrowOutControls = sourceThrowOutControlDictionary.GetValueOrDefault(route.Id, []);

            var result = ProcessSignalControl(route, directLockCondition, signalControlCondition,
                routeLockTrackCircuit, targetThrowOutControls, sourceThrowOutControls, interlockingObjects);
            // 変更があれば、DBに保存する
        ...
        Dictionary<ulong, InterlockingObject> interlockingObjects)
    {
        ...
        // てこナシで総括制御される場合、総括制御する側の進路鎖錠リレーが扛上していた場合、信号制御リレーを落下させる
        if (sourceThrowOutControls.Any(toc =>
            {
                var sourceRoute = interlockingObjects.GetValueOrDefault(toc.SourceId) as Route;
                return toc.ControlType == ThrowOutControlType.WithoutLever &&
                       sourceRoute?.RouteState.IsRouteLockRaised == RaiseDrop.Raise;
            }))
        {
            return RaiseDrop.Drop;
        }
        ...
        LockConditionObject o, InterlockingObject interlockingObject)
    {
        ...
        return interlockingObject switch
        {
            ...
            DirectionRoute directionRoute => o.IsLR == LR.Left
                ? directionRoute.DirectionRouteState.IsLRelayRaised == RaiseDrop.Raise
                : directionRoute.DirectionRouteState.IsRRelayRaised == RaiseDrop.Raise,
            _ => false
        };
    }
...
public class LeverRelayState
{
    ...
    /// X総括リレー(てこナシ)
    /// </summary>
    public RaiseDrop IsXRelayRaised { get; set; }
