...
(
    ...
    -- 時素条件成立。連動サーバーが書き、公開側は読むのみ
    is_timer_condition_met BOOLEAN  NOT NULL DEFAULT false,
    UNIQUE (station_id, seconds)
);
-- 注意: このテーブルは2プロセス(公開サーバー / 連動サーバー)が別々の列を書く。
--       全カラム上書き(EFのUpdate)は禁止。必ず列限定更新(ExecuteUpdate + SetProperty)で書くこと。
...
(
    ...
    is_inserted_key     BOOL       NOT NULL DEFAULT 'false',                            -- 鍵が挿入されているか
    is_reversed         nr         NOT NULL DEFAULT 'normal',                           -- てこの位置
    is_center_controlled BOOLEAN   NOT NULL DEFAULT false                               -- 集中制御中(true)か駅扱い(false)か
);
-- 注意: このテーブルは2プロセス(公開サーバー / 連動サーバー)が別々の列を書く。
--       全カラム上書き(EFのUpdate)は禁止。必ず列限定更新(ExecuteUpdate + SetProperty)で書くこと。
...
(
    ...
    is_route_secured         raise_drop NOT NULL DEFAULT 'drop',       -- 進路確保中か
    is_ctc_controlled        raise_drop NOT NULL DEFAULT 'drop'        -- CTC制御中か(CTC制御盤からの操作入力)
);
-- 注意: このテーブルは2プロセス(公開サーバー / 連動サーバー)が別々の列を書く。
--       全カラム上書き(EFのUpdate)は禁止。必ず列限定更新(ExecuteUpdate + SetProperty)で書くこと。
...
(
    ...
    interlocking_heartbeat_at   TIMESTAMP             NULL     DEFAULT NULL,           -- 連動サーバーの生存確認用
