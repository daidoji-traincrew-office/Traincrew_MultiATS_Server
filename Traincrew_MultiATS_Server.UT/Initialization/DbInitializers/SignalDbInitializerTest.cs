...
public class SignalDbInitializerTest
{
    ...
    public async Task InitializeSignalsAsync_ShouldAddSignals_WhenDataIsValid()
    {
        ...
        _generalRepositoryMock.Verify(
            r => r.AddAll(It.Is<List<Signal>>(list =>
                list.Count == 1 &&
                list[0].Name == "Signal1" &&
                list[0].TypeName == "Type1" &&
    ...
    public async Task InitializeSignalsAsync_ShouldSkipExistingSignals()
    {
        ...
        // Assert
        _generalRepositoryMock.Verify(
            r => r.AddAll(It.Is<List<Signal>>(list => list.Count == 0), It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task InitializeSignalsAsync_ShouldThrowException_WhenDirectionRouteNotFound()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(
            async () => await initializer.InitializeSignalsAsync(signalCsvList, TestContext.Current.CancellationToken));
    }
    ...
    public async Task InitializeNextSignalsAsync_ShouldAddNextSignals_WhenDataIsValid()
    {
        ...
        // Assert
        _generalRepositoryMock.Verify(
            r => r.AddAll(It.Is<List<NextSignal>>(list =>
                list.Count == 1 &&
                list[0].SignalName == "Signal1" &&
                list[0].SourceSignalName == "Signal1" &&
                list[0].TargetSignalName == "Signal2" &&
                list[0].Depth == 1
            ), It.IsAny<CancellationToken>()),
            Times.AtLeastOnce);
    }
    ...
    public async Task InitializeSignalRoutesAsync_ShouldAddSignalRoutes_WhenDataIsValid()
    {
        ...
        // Assert
        _generalRepositoryMock.Verify(
            r => r.AddAll(It.Is<List<SignalRoute>>(list =>
                list.Count == 1 &&
                list[0].SignalName == "Signal1" &&
                list[0].RouteId == 1
            ), It.IsAny<CancellationToken>()),
    ...
    public async Task InitializeSignalRoutesAsync_ShouldSkipExistingRoutes()
    {
        ...
        // Assert
        _generalRepositoryMock.Verify(
            r => r.AddAll(It.Is<List<SignalRoute>>(list => list.Count == 0), It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    public async Task InitializeAsync_ShouldCompleteWithoutAction()
    {
        ...
        // Assert
        _generalRepositoryMock.Verify(
            r => r.AddAll(It.IsAny<List<Signal>>(), It.IsAny<CancellationToken>()),
            Times.Never);
    }
