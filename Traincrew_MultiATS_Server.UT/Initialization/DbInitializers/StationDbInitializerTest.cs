...
public class StationDbInitializerTest
{
    ...
    public async Task InitializeStationTimerStatesAsync_ShouldAddTimerStates_ForEachStation()
    {
        ...
        _generalRepositoryMock.Verify(
            r => r.AddAll(It.Is<List<StationTimerState>>(list =>
                ...
                // リレー行(連動側専有テーブル)は連動側(Interlocking.Server)へ移設され、
                // 公開側の StationTimerState はリレー行の初期化責務を持たない
                list.All(s => s.IsTimerConditionMet == false)
            ), It.IsAny<CancellationToken>()),
            Times.Once);
    }
    ...
    [Fact]
    public async Task InitializeStationTimerStatesAsync_ShouldSetCorrectDefaultIsTimerConditionMet()
    {
        // Arrange
        var stationIds = new List<string> { "ST1" };
        ...
        // リレー行(連動側専有テーブル)は連動側(Interlocking.Server)へ移設された
        // (公開側の StationTimerState はリレー行の初期化責務を持たない)

        // 親 StationTimerState.IsTimerConditionMet は ten == Drop && ter == Drop を畳んだ値。
        // 既定値では ten=Drop, ter=Raise のため false が期待される。
        Assert.False(capturedState30.IsTimerConditionMet);
        Assert.False(capturedState60.IsTimerConditionMet);
    }
}
